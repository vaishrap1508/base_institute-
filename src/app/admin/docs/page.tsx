        ...
        {currentRole.role !== 'admin' ? (
              ...
              <div className="w-full bg-slate-50 border border-slate-200/80 p-4 rounded-xl space-y-3.5 text-xs text-left">
                ...
                <div className="grid grid-cols-2 gap-y-3.5 gap-x-6 font-semibold">
                  ...
                  <div className="flex flex-col col-span-2">
                    ...
                    <span className="text-slate-800 font-bold font-mono text-[11px]">/admin/documentation</span>
                  </div>
                </div>
              </div>
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-6">
            ...
            <div className="border-b border-slate-200/60 pb-5">
              <h1 className="text-2xl font-black text-slate-900 tracking-tight">Aptitude API Specifications</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 bg-white border border-slate-200/90 rounded-2xl shadow-xs overflow-hidden flex flex-col">
                <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/40 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <FileCode className="w-4.5 h-4.5 text-slate-500" />
                    <span className="text-xs font-bold text-slate-800 tracking-tight">Endpoint Registry v2</span>
                  </div>
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse text-left">
                    ...
                    <tbody className="divide-y divide-slate-150 text-xs font-medium text-slate-700">
                      {apis.map((api) => (
                        <tr key={api.path} className="hover:bg-slate-50/40 transition-colors">
                          <td className="px-6 py-4.5">
                            <div className="flex items-center gap-2">
                              {api.method === 'GET' ? (
                                <span className="bg-emerald-50 border border-emerald-100 text-emerald-700 text-[10px] font-extrabold px-2 py-0.5 rounded">GET</span>
                              ) : (
                                <span className="bg-blue-50 border border-blue-100 text-blue-700 text-[10px] font-extrabold px-2 py-0.5 rounded">POST</span>
                              )}
                              <span className="font-mono font-bold text-slate-800 text-[11px]">{api.path}</span>
                            </div>
                          </td>
                          <td className="px-6 py-4.5 text-slate-500 leading-relaxed font-medium">{api.desc}</td>
                          <td className="px-6 py-4.5 font-semibold text-slate-400">{api.auth}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {/* JSON Response Panel */}
              <div className="bg-white border border-slate-200/90 rounded-2xl shadow-xs p-6 space-y-4 flex flex-col justify-between">
                <div className="space-y-4">
                  <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                    <div className="flex items-center gap-2">
                      <BookOpen className="w-4.5 h-4.5 text-blue-600" />
                      <h3 className="text-sm font-bold text-slate-800 tracking-tight">Structured Item Schema</h3>
                    </div>
                  </div>

                  {/* Dark Mode Fenced Code Box */}
                  <div className="bg-slate-900 rounded-xl p-4 font-mono text-[10px] leading-relaxed text-indigo-200 overflow-x-auto shadow-inner relative group">
                    <pre>{sampleJson}</pre>
                ...
                <div className="pt-4 border-t border-slate-100 flex items-center gap-3 text-[11px] text-slate-400">
