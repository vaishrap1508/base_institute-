...
// ---------------------------------------------------------
interface EnrichedTopicProgress extends TopicGridItem {
  companies: string[];
...
export default function DomainDetailPage() {
  ...
  const [topics, setTopics] = useState<EnrichedTopicProgress[]>([]);
  ...
  useEffect(() => {
    ...
    const loadSessionAndData = async () => {
      try {
        ...
        const metadataMap: Record<string, { companies: string[]; difficulty: 'Easy' | 'Medium' | 'Hard' }> = {
          'percentages': { companies: ['TCS', 'Infosys'], difficulty: 'Easy' },
          'profit-loss': { companies: ['TCS', 'Accenture', 'Infosys'], difficulty: 'Medium' },
          'time-work': { companies: ['Amazon', 'Accenture'], difficulty: 'Medium' },
          'algebra': { companies: ['Amazon', 'Google'], difficulty: 'Hard' },
          'geometry': { companies: ['Amazon', 'Google'], difficulty: 'Hard' },
          'ratio-prop': { companies: ['TCS', 'Infosys', 'Accenture'], difficulty: 'Easy' },
          'arithmetic': { companies: ['TCS', 'Infosys', 'Accenture'], difficulty: 'Easy' },
          'linear-arr': { companies: ['Infosys', 'Accenture'], difficulty: 'Medium' },
          'circular-arr': { companies: ['Amazon', 'Google'], difficulty: 'Hard' },
          'arrangements': { companies: ['Infosys', 'Accenture', 'Amazon'], difficulty: 'Medium' },
          'grammar': { companies: ['TCS', 'Infosys'], difficulty: 'Easy' },
          'tenses': { companies: ['TCS', 'Infosys'], difficulty: 'Easy' },
          'structures': { companies: ['TCS', 'Amazon', 'Google'], difficulty: 'Easy' },
          'arrays': { companies: ['TCS', 'Amazon', 'Google'], difficulty: 'Easy' },
          'algorithms': { companies: ['Amazon', 'Google'], difficulty: 'Hard' },
          'recursion': { companies: ['Amazon', 'Google'], difficulty: 'Hard' }
        };

        const enrichedTopics = topicsRes.map(t => {
          const meta = metadataMap[t.id] || { companies: ['TCS'], difficulty: 'Easy' as const };
          return {
            ...t,
  ...
  // Filter topics list by search, company, and difficulty
  ...
  }, [topics, searchQuery, selectedCompany, selectedDifficulty]);
            ...
            {loading ? (
              ...
              <div className="space-y-6 animate-fadeIn">
                    ...
                    {continueLearning && (
                      <div className="bg-white border border-slate-200/80 dark:bg-slate-900/40 dark:border-slate-900/80 rounded-2xl p-5 flex flex-col justify-between gap-4 hover:border-[var(--clr-primary)]/40 transition-all duration-300 text-left">
                        ...
                        <div className="space-y-2.5">
                          ...
                          {/* Dynamic Next Up tip */}
                          <div className="p-2.5 bg-[var(--clr-primary)]/5 border border-[var(--clr-primary)]/10 rounded-xl text-[10px] text-slate-550 dark:text-slate-400 leading-normal font-semibold">
                            💡 <strong>Streak Saver:</strong> You haven't touched {continueLearning.name.split(' ')[0]} in 3 days. We recommend solving 5 "Easy" questions to keep your placements streak alive!
                          </div>
                ...
                <div className="space-y-4">
                  <div className="sticky top-0 bg-slate-50/90 dark:bg-[#030712]/90 backdrop-blur-md py-4 z-30 border-b border-slate-200/40 dark:border-slate-900/60 select-none space-y-4">
                    ...
                    {/* Company Power-Filters and Difficulty toggles */}
                    <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between text-left">
                      {/* Company Tags */}
                      <div className="flex flex-wrap items-center gap-1.5">
                        <span className="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider mr-1">Goal Filter:</span>
                        {['All', 'TCS', 'Infosys', 'Accenture', 'Amazon', 'Google'].map(c => (
                          <button
                            key={c}
                            onClick={() => setSelectedCompany(c)}
                            className={`px-2.5 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all cursor-pointer border-0 ${
                              selectedCompany === c
                                ? 'bg-[var(--clr-primary)] text-white shadow-xs'
                                : 'bg-white hover:bg-slate-100 dark:bg-slate-900 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400'
                            }`}
                          >
                            {c}
                          </button>
                      ...
                      {/* Difficulty Toggles */}
                      <div className="flex flex-wrap items-center gap-1.5">
                        <span className="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider mr-1">Difficulty:</span>
                          ...
                          <button
                            ...
                            className={`px-2.5 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all cursor-pointer border-0 ${
                              selectedDifficulty === d
                                ...
                                : 'bg-white hover:bg-slate-100 dark:bg-slate-900 dark:hover:bg-slate-800 text-slate-505 dark:text-slate-400'
                            }`}
                          >
                            {d}
                          </button>
                      ...
                      filteredTopics.map((topic) => {
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-start justify-between gap-3 text-left">
                              <div className="space-y-2">
                                <div className="flex flex-wrap items-center gap-2">
                                  ...
                                  <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-md border ${
                                    (topic.difficulty || 'Easy') === 'Easy'
                                      ? 'bg-emerald-50 border-emerald-100 dark:bg-emerald-950/20 dark:border-emerald-900/40 text-emerald-600 dark:text-emerald-400'
                                      : (topic.difficulty || 'Easy') === 'Medium'
                                      ? 'bg-amber-50 border-amber-100 dark:bg-amber-950/20 dark:border-amber-900/40 text-amber-600 dark:text-amber-450'
                                      : 'bg-rose-50 border-rose-100 dark:bg-rose-950/20 dark:border-rose-900/40 text-rose-600 dark:text-rose-400'
                                  }`}>
                                ...
                                <div className="flex flex-wrap gap-1">
                                  {(topic.companies || []).map(co => (
                                    <span key={co} className="text-[8px] font-black font-mono bg-slate-100 dark:bg-slate-900 text-slate-500 dark:text-slate-400 border border-slate-200/45 dark:border-slate-800/80 px-2 py-0.5 rounded">
                                      {co}
                                    </span>
                                  ))}
                                </div>

                                <div className="flex items-center gap-2 text-[10px] font-mono text-slate-400 dark:text-slate-500 select-none pt-1">
                            ...
                            {/* Visual Progress Grid (LeetCode/GitHub activity style) */}
                            <div className="space-y-1.5 select-none text-left">
                              <span className="text-[8px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block font-mono">Activity Commits</span>
                              <div className="flex flex-wrap gap-1">
                                {Array.from({ length: 24 }).map((_, idx) => {
                                  const percentThreshold = (idx / 24) * 100;
                                  const isDotSolved = topic.progress > percentThreshold;
                                  return (
                                    <div
                                      key={idx}
                                      className={`w-2.5 h-2.5 rounded-xs border transition-colors ${
                                        isCompleted
                                          ? 'bg-emerald-500 border-emerald-500'
                                          : isDotSolved
                                          ? 'bg-[var(--clr-primary)] border-[var(--clr-primary)]'
                                          : 'bg-slate-100 dark:bg-slate-950/80 border-slate-200 dark:border-slate-900'
                                      }`}
                                      title={`Progress Checkpoint ${idx + 1}`}
                                    />
                                  );
                                })}
                              </div>
                              <div className="flex items-center justify-between text-[8px] font-bold text-slate-450 dark:text-slate-500 uppercase tracking-wider pt-0.5">
                                <span>Completeness</span>
